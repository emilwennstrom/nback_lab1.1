 ...
 */
void
createNBackString(int nBackString[], int size, int combinations, int matchPercentage, int nback);
...
void fillInAMatch(int nBackString[], int size, int combinations, int nback) {
    ...
    while (test == 0) {
        ...
        if ((possition + nback < size) && nBackString[possition] == 0 &&
            nBackString[possition + nback] == 0) {
